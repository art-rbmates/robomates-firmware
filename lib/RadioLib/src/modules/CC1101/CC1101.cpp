...
int16_t CC1101::packetMode() {
  int16_t state = SPIsetRegValue(RADIOLIB_CC1101_REG_PKTCTRL1, RADIOLIB_CC1101_CRC_AUTOFLUSH_ON | RADIOLIB_CC1101_APPEND_STATUS_ON | RADIOLIB_CC1101_ADR_CHK_NONE, 3, 0); // MYFIX: use RADIOLIB_CC1101_CRC_AUTOFLUSH_ON instead of RADIOLIB_CC1101_CRC_AUTOFLUSH_OFF for better reliability
  state |= SPIsetRegValue(RADIOLIB_CC1101_REG_PKTCTRL0, RADIOLIB_CC1101_WHITE_DATA_ON | RADIOLIB_CC1101_PKT_FORMAT_NORMAL, 6, 4); // MYFIX: use RADIOLIB_CC1101_WHITE_DATA_ON instead of RADIOLIB_CC1101_WHITE_DATA_OFF for better reliability
...
void CC1101::setPacketSentAction(void (*func)(void)) {
  this->setGdo2Action(func, this->mod->hal->GpioInterruptRising);
}
...
int16_t CC1101::startTransmit(const uint8_t* data, size_t len, uint8_t addr) {
  ...
  // MYFIX: TX-if-CCA path in startTransmit() begin
  // Reset CCA diagnostic flag
  lastTxBlockedByCca = false;

  // TX-if-CCA path: when enabled, use CCA check before transmitting
  if(txIfCcaEnabled) {
    // 1) Go IDLE, clear TX FIFO
    standby();
    SPIsendCommand(RADIOLIB_CC1101_CMD_FLUSH_TX);

    // 2) Enter RX (required: STX while in RX uses TX-if-CCA)
    this->mod->setRfSwitchState(Module::MODE_RX);
    SPIsendCommand(RADIOLIB_CC1101_CMD_RX);

    // Wait for MARCSTATE == RX (0x0D)
    {
      RadioLibTime_t start = this->mod->hal->micros();
      while(SPIgetRegValue(RADIOLIB_CC1101_REG_MARCSTATE, 4, 0) != RADIOLIB_CC1101_MARC_STATE_RX) {
        if(this->mod->hal->micros() - start > 5000) {
          standby();
          return(RADIOLIB_ERR_TX_TIMEOUT);
        }
      }
    ...
    // RSSI/CCA needs a moment after entering RX to become valid
    this->mod->hal->delay(1);

    // 3) Configure GDO mapping like existing code does
    int16_t state = RADIOLIB_ERR_NONE;
    if(len <= RADIOLIB_CC1101_FIFO_SIZE) {
      state = SPIsetRegValue(RADIOLIB_CC1101_REG_IOCFG2, RADIOLIB_CC1101_GDOX_PA_PD, 5, 0);
      RADIOLIB_ASSERT(state);
    }

    // 4) Load FIFO (same logic as existing implementation)
    uint8_t dataSent = 0;
    ...
    if(this->packetLengthConfig == RADIOLIB_CC1101_LENGTH_CONFIG_VARIABLE) {
      if(len > RADIOLIB_CC1101_MAX_PACKET_LENGTH - 1) return(RADIOLIB_ERR_PACKET_TOO_LONG);
    ...
    // 5) CCA retry loop: check PKTSTATUS.CCA, then STX while in RX
    for(uint8_t attempt = 0; attempt <= txIfCcaMaxRetries; attempt++) {

      // PKTSTATUS.CCA = 1 means channel clear
      uint8_t pktStatus = SPIreadRegister(RADIOLIB_CC1101_REG_PKTSTATUS);
      bool ccaClear = (pktStatus & RADIOLIB_CC1101_CCA) != 0;

      if(!ccaClear) {
        lastTxBlockedByCca = true;
        ccaBlockedTxCount++;

      } else {
        // Switch RF path to TX right before strobe
        this->mod->setRfSwitchState(Module::MODE_TX);

        // STX while in RX triggers TX-if-CCA
        SPIsendCommand(RADIOLIB_CC1101_CMD_TX);

        // Give it a moment to transition
        this->mod->hal->delayMicroseconds(200);

        uint8_t marc = SPIgetRegValue(RADIOLIB_CC1101_REG_MARCSTATE, 4, 0);
        if(marc == RADIOLIB_CC1101_MARC_STATE_TX || marc == RADIOLIB_CC1101_MARC_STATE_FSTXON) {
          // TX (0x13) or FSTXON (0x12) => transmit started
          // Feed FIFO remainder (same as existing code)
          while(dataSent < len) {
            uint8_t fifoBytes = 0;
        ...
        // If we got here: STX did not start TX => treated as CCA-blocked
        lastTxBlockedByCca = true;
        ccaBlockedTxCount++;

        // Go back to RX path for retry
        this->mod->setRfSwitchState(Module::MODE_RX);
        SPIsendCommand(RADIOLIB_CC1101_CMD_RX);
      }

      // Backoff with cap; no infinite loops
      if(attempt == txIfCcaMaxRetries) break;

      uint16_t exp = attempt > 6 ? 6 : attempt; // cap exponent
      uint16_t slots = (1u << exp);
      uint32_t jitter = (this->mod->hal->micros() ^ (this->mod->hal->millis() << 16));
      uint16_t chosen = (uint16_t)(jitter % slots);
      uint16_t backoffMs = (uint16_t)(txIfCcaBaseBackoffMs * chosen);

      this->mod->hal->delay(backoffMs);
    }

    // Drop packet: channel stayed busy through retries
    standby();
    return RADIOLIB_ERR_UNKNOWN;  // distinguish using wasLastTxBlockedByCca()
  }
  // MYFIX: TX-if-CCA path in startTransmit() end

  // --- existing implementation below (standby/FSTXON/TX path) ---
  ...
  // Increased timeout for reliability when other high-priority tasks are running
  RadioLibTime_t start = this->mod->hal->micros();
  while(SPIgetRegValue(RADIOLIB_CC1101_REG_MARCSTATE, 4, 0) != 0x12) {
    if(this->mod->hal->micros() - start > 5000) {  // MYFIX: 5ms timeout (was 800us) - increased for reliability when balance controller preempts SPI
      standby();
      return(RADIOLIB_ERR_TX_TIMEOUT);
    }
  }
  ...
  int16_t state = RADIOLIB_ERR_NONE;
  if(len <= RADIOLIB_CC1101_FIFO_SIZE) {
    state = SPIsetRegValue(RADIOLIB_CC1101_REG_IOCFG2, RADIOLIB_CC1101_GDOX_PA_PD, 5, 0); // MYFIX RADIOLIB_CC1101_GDOX_SYNC_WORD_SENT_OR_PKT_RECEIVED before
    RADIOLIB_ASSERT(state);
  }
...
int16_t CC1101::finishTransmit() {
  ...
  // RadioLibTime_t timeout = (1.0f/(this->bitRate))*(RADIOLIB_CC1101_FIFO_SIZE*2.0f); // MYFIX: bytes -> bits
  // 2x FIFO (bytes) * 8 bits/byte = bits
  // divide by bitRate (bits/ms) => ms
  RadioLibTime_t timeout = (RadioLibTime_t)5 + (RadioLibTime_t)((RADIOLIB_CC1101_FIFO_SIZE * 2UL * 8UL) / this->bitRate);
...
int16_t CC1101::startReceive() {
  ...
  state = SPIsetRegValue(RADIOLIB_CC1101_REG_IOCFG0, RADIOLIB_CC1101_GDOX_PKT_RECEIVED_CRC_OK, 6, 0); // MYFIX: use RADIOLIB_CC1101_GDOX_PKT_RECEIVED_CRC_OK instead of RADIOLIB_CC1101_GDOX_RX_FIFO_FULL_OR_PKT_END for better reliability
  RADIOLIB_ASSERT(state);
...
int16_t CC1101::setRxBandwidth(float rxBw) {
  ...
  for(int8_t e = 3; e >= 0; e--) {
    for(int8_t m = 3; m >= 0; m --) {
      ...
      // if(fabs((rxBw * 1000.0f - point) <= 1000.0f)) { // MYFIX: parentheses bug
...
int16_t CC1101::getFrequencyDeviation(float *freqDev) {
  ...
  //
  // *freqDev = (1000.0f / (uint32_t(1) << 17)) - (8 + m) * (uint32_t(1) << e); // MYFIX: math is wrong: - instead of *, and wrong constants

  float fXosc = RADIOLIB_CC1101_CRYSTAL_FREQ * 1000000.0f;     // Hz
  float devHz = (fXosc / (1UL << 17)) * (8 + m) * (1UL << e);  // Hz
  *freqDev = devHz / 1000.0f;                                  // kHz
...
// MYFIX: CCA / TX-if-CCA method implementations begin
int16_t CC1101::setCcaMode(uint8_t ccaMode) {
  // MCSM1.CCA_MODE is bits [5:4]
  return SPIsetRegValue(RADIOLIB_CC1101_REG_MCSM1, ccaMode, 5, 4);
}

int16_t CC1101::setCarrierSenseThreshold(int8_t absThrDb, uint8_t relThr) {
  // AGCCTRL1.CARRIER_SENSE_REL_THR is [5:4], ABS_THR is [3:0]
  // ABS_THR is 4-bit 2's complement, -8 disables
  if(absThrDb < -8) absThrDb = -8;
  if(absThrDb >  7) absThrDb =  7;

  uint8_t absNibble;
  if(absThrDb < 0) absNibble = (uint8_t)((absThrDb + 16) & 0x0F);
  else             absNibble = (uint8_t)(absThrDb & 0x0F);

  int16_t state = SPIsetRegValue(RADIOLIB_CC1101_REG_AGCCTRL1, relThr, 5, 4);
  state |= SPIsetRegValue(RADIOLIB_CC1101_REG_AGCCTRL1, absNibble, 3, 0);
  return state;
}

void CC1101::enableTxIfCca(bool enable, uint8_t maxRetries, uint16_t baseBackoffMs) {
  txIfCcaEnabled = enable;
  txIfCcaMaxRetries = maxRetries;
  txIfCcaBaseBackoffMs = baseBackoffMs;
}

bool CC1101::wasLastTxBlockedByCca() const {
  return lastTxBlockedByCca;
}

uint32_t CC1101::getCcaBlockedTxCount() const {
  return ccaBlockedTxCount;
}
// MYFIX: CCA / TX-if-CCA method implementations end
...
int16_t CC1101::beginCommon(float freq, float br, float freqDev, float rxBw, int8_t pwr, uint8_t preambleLength) {
  ...
  state = setEncoding(RADIOLIB_ENCODING_WHITENING);
  RADIOLIB_ASSERT(state);
