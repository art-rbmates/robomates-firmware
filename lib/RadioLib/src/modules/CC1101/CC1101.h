...
class CC1101: public PhysicalLayer {
  public:
    ...
    */
    ...
    // MYFIX: CCA / TX-if-CCA API begin
    // --- CCA / TX-if-CCA helpers ---
    
    /*!
      \brief Set CCA (Clear Channel Assessment) mode.
      \param ccaMode CCA mode to use. Options:
        - RADIOLIB_CC1101_CCA_MODE_ALWAYS (0x00): Always give clear channel indication
        - RADIOLIB_CC1101_CCA_MODE_RSSI_THR (0x10): RSSI below threshold
        - RADIOLIB_CC1101_CCA_MODE_RX_PKT (0x20): Unless currently receiving a packet
        - RADIOLIB_CC1101_CCA_MODE_RSSI_THR_RX_PKT (0x30): RSSI below threshold unless receiving packet (default)
      \returns \ref status_codes
    */
    int16_t setCcaMode(uint8_t ccaMode);

    /*!
      \brief Set carrier sense threshold for CCA.
      \param absThrDb Absolute RSSI threshold in dB relative to MAGN_TARGET (-8 to +7, -8 disables).
      \param relThr Relative threshold (CARRIER_SENSE_REL_THR). Defaults to OFF.
      \returns \ref status_codes
    */
    int16_t setCarrierSenseThreshold(int8_t absThrDb, uint8_t relThr = RADIOLIB_CC1101_CARRIER_SENSE_REL_THR_OFF);

    /*!
      \brief Enable TX-if-CCA behavior inside startTransmit().
      When enabled, startTransmit() will check CCA before transmitting and back off if channel is busy.
      \param enable Enable (true) or disable (false) TX-if-CCA.
      \param maxRetries Maximum number of CCA retries before giving up (default 6).
      \param baseBackoffMs Base backoff time in milliseconds for exponential backoff (default 5).
    */
    void enableTxIfCca(bool enable, uint8_t maxRetries = 6, uint16_t baseBackoffMs = 5);

    /*!
      \brief Check if the last transmission was blocked by CCA.
      \returns true if last TX was blocked, false otherwise.
    */
    bool wasLastTxBlockedByCca() const;

    /*!
      \brief Get total count of transmissions blocked by CCA.
      \returns Number of TX attempts blocked by CCA.
    */
    uint32_t getCcaBlockedTxCount() const;
    // MYFIX: CCA / TX-if-CCA API end
    ...
    // MYFIX: TX-if-CCA private members begin
    // TX-if-CCA configuration
    bool txIfCcaEnabled = false;
    uint8_t txIfCcaMaxRetries = 6;
    uint16_t txIfCcaBaseBackoffMs = 5;

    // TX-if-CCA diagnostic state
    volatile bool lastTxBlockedByCca = false;
    uint32_t ccaBlockedTxCount = 0;
    // MYFIX: TX-if-CCA private members end
